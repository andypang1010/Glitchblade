#include "GBBoss2Controller.h"

#define SIGNUM(x)  ((x > 0) - (x < 0))

Boss2Controller::Boss2Controller() : EnemyController() {}

void Boss2Controller::init(const std::shared_ptr<AssetManager>& assetRef,
    const std::shared_ptr<JsonValue>& enemiesJSON,
    std::vector<std::shared_ptr<ActionModel>> actions) {
    _enemyJSON = enemiesJSON->get("boss_2");
    _enemy = Boss2Model::alloc(assetRef, _enemyJSON, ENEMY_DEFAULT_INIT_POS, actions);
    EnemyController::init(assetRef, _enemyJSON, actions);
}

void Boss2Controller::applyForce() {
    if (!_enemy || !_enemy->isEnabled()) {CULog("no enemy boss 2"); return;}
...
void Boss2Controller::postUpdate(float dt) {

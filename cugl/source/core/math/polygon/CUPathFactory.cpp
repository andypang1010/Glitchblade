 ...
 * @return a reference to the buffer for chaining.
 */
Path2* PathFactory::makeCircle(Path2* path, float cx, float cy, float radius) const {
    return makeEllipse(path, cx, cy, 2*radius, 2*radius);
}
 ...
 */
Path2 PathFactory::makeArc(float cx, float cy, float radius, float start, float degrees, bool closed) const {
    Path2 result;
    makeArc(&result,cx,cy,radius,start,degrees,closed);
    return result;
}
 ...
 * @return a reference to the buffer for chaining.
 */
Path2* PathFactory::makeArc(Path2* path, float cx, float cy, float radius, float start, float degrees, bool closed) const {
    CUAssertLog(path != nullptr, "Path buffer is null");
    CUAssertLog(degrees>0 && degrees<=360, "Degree out of range");
    Uint32 offset = (Uint32)path->vertices.size();
    ...
    Uint32 size = (degrees==360 || !closed) ? offset+segments+1 : offset+segments+2;
    path->reserve(size);
    Vec2 vert;
 ...
 * @return a reference to the buffer for chaining.
 */
Path2* PathFactory::makeCapsule(Path2* path, float x, float y, float w, float h) const {
    return makeCapsule(path, poly2::Capsule::FULL, x, y, w, h);
}

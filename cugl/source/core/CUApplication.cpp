...
bool Application::step() {
	...
	// Sleep the remainder
    current.mark();
    Uint32 millis = (Uint32)current.ellapsedMillis(_finish);
    if (millis < _delay) {
...
void Application::setMultiSampled(bool flag) {
	CUAssertLog(_state == State::NONE, "Cannot reset application display after initialization");
    CUAssertLog(Display::supportsMultisample(), "Multisampling is not supported in OpenGLES");
	_multisamp = flag;
}
...
/**
 * Returns a description of this graphics API for this application.
 *
 ...
 * "Headless".
 *
 * @return a description of this graphics API for this application.
 */
const std::string Application::getGraphicsDescription() const {
    return Display::get()->getGraphicsDescription();
}
